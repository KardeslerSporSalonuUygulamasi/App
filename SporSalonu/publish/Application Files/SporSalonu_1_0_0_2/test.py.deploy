...
class Uyeler:
	...
	def kayit(Id, adSoyad, yas, cinsiyet, boy, kilo, telefon, emailadress, dogumTarihi, program):
	...
	#constructor
	def __init__(self, Id, adSoyad, yas, cinsiyet, boy, kilo, telefon, emailadress, dogumTarihi, program):
		self.Id = Id
	...
	def serialize(self):#üye bilgilerinin yazıya dökümü
		return [str(self.Id), self.adSoyad, self.yas, self.cinsiyet, self. boy, self.kilo, str(self.telefon), self.emailadress, str(self.dogumTarihi), self.program]
	...
	def arama(aranan):#üye arama
		...
		if not(aranan.isdigit() or aranan.istitle()):
			#print("if")
